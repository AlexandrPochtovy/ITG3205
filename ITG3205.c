...
uint8_t ITG3205_GetData(I2C_Connection *_i2c, ITG3205_t *dev) {
	...
	if (_i2c->status == PORT_FREE) {
		...
		case 0:
			...
			break;
		case 1:
			GetMulti(&_i2c->buffer, dt, ITG3205_DATA_LEN);
			dev->raw.temp = (int16_t) (((uint16_t) dt[0]) << 8
					| (uint16_t) dt[1]); //CONCAT_BYTES(dt[0], dt[1]);
			...
			dev->data.temp = 35
					+ ((float) (dev->raw.temp + 13200)) / Chip_TEMP_LSB;
